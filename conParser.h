void parseRigidCondensed(ifstream &ifs, string &s, vector3d &offset, int counter, quaternion &orient3, simulation &sim, int idx)
{
...
//original parseRigid starts here
	quaternion orient;
	...
	{
		getline(ifs, s); 
		if (s[0] == 'r' || s[0] == '!' || s[0] == 'p' || s[0] == 'd' || (s[0] == 's' && s[1] == 'c')) break;
		if (s[0] != ';')
		{
			//cout << s << '\n';
			m = 1;
...
void parseIntCondensed(ifstream &ifs, string &s, vector3d &offset, int counter, quaternion &rotate, simulation &sim, int &intIdx, int steps)
{
	rigidBody * RBlist = sim.RBlist;
	istringstream iss(s);
	//cout << s << '\n';
	...
	if (s == "constraint")
	{
		...
		constraint * c = new constraint(vp[0], locus, ground, k, RBlist); sim.condensedIntList[intIdx] = c; intIdx++;
	}
	else if (s == "constrainAxis")
	{
		getline(ifs, s); istringstream iss2(s);
		vector3d locus;
		...
		vp[0].second++; vp[0].first += counter;
		vp[0].second = massf(RBlist, vp, 0);
		constrainAxis * c = new constrainAxis(vp[0], locus, ground, k, RBlist); sim.condensedIntList[intIdx] = c; intIdx++;
	}
	else if (s == "constrainAxisTime")
	{
		getline(ifs, s); istringstream iss2(s);
	...
	else if (s == "bond" || s == "chain")
	{
		...
		string oa;
		bond * b = new bond(vp[0], vp[1], ground, k, RBlist); sim.condensedIntList[intIdx] = b; intIdx++;
		iss2 >> oa; if (oa == "oa") iss2 >> b->angleType.first >> b->angleType.second;
		b->isChain = bisChain;
		bond temp = *b;
	...
	else if (s == "angle")
	{
		...
		angle * a = new angle(vp[0], vp[1], vp[2], ground, k, RBlist); a->obs = observe; sim.condensedIntList[intIdx] = a; intIdx++;
	}
	/*else if (s == "quartAngle")
	{
		...
		quartAngle * a = new quartAngle(vp[0], vp[1], vp[2], ground, k, RBlist); a->obs = observe; sim.condensedIntList[intIdx] = a; intIdx++;
	}*/
	else if (s == "gaussAngle")
	{
		...
		gaussAngle * a = new gaussAngle(vp[0], vp[1], vp[2], ground, k, RBlist); a->obs = observe; sim.condensedIntList[intIdx] = a; intIdx++;
	}
	else if (s == "angleAxis")
	{
		getline(ifs, s); istringstream iss2(s);
		...
		angleAxis * a = new angleAxis(vp[0], vp[1], axis, ground, k, RBlist); a->obs = observe; sim.condensedIntList[intIdx] = a; intIdx++;
	}
	else if (s == "dihedral")
	{
		iss >> divisor;
		...
		dihedral * d = new dihedral(vp[0], vp[1], vp[2], vp[3], ground, k, divisor, RBlist); sim.condensedIntList[intIdx] = d; intIdx++;
	}
	else if (s == "extDipole")
	{
		getline(ifs, s); istringstream iss2(s);
		...
		vp[0].second++; vp[0].first += counter;
		vp[0].second = massf(RBlist, vp, 0);
		extDipole * c = new extDipole(vp[0], &sim.externalDipole, &sim.prevDipole, &sim.expenditure, ground, k, RBlist, 0, sim.rotationAxis); sim.condensedIntList[intIdx] = c; intIdx++;
	}
	else if (s == "extDipoleShift")
	{
		...
		vp[0].second++; vp[0].first += counter;
		vp[0].second = massf(RBlist, vp, 0);
		extDipole * c = new extDipole(vp[0], &sim.externalDipole, &sim.prevDipole, &sim.expenditure, ground, k, RBlist, angle, sim.rotationAxis); sim.condensedIntList[intIdx] = c; intIdx++;
	}
	else if (s == "staccatoDipole")
	{
		...
		sim.condensedIntList[intIdx] = c; intIdx++;
	}
	else if (s == "offDipole")
	{
		double offset = 0;
		...
		vp[0].second++; vp[0].first += counter;
		vp[0].second = massf(RBlist, vp, 0);
		offDipole * c = new offDipole(vp[0], &sim.offsetDipole, &sim.prevOffDipole, &sim.expenditure, ground, k, RBlist, 0, sim.rotationAxis); sim.condensedIntList[intIdx] = c; intIdx++;
	}
	else if (s == "offDipoleShift")
	{
		...
		vp[0].second++; vp[0].first += counter;
		vp[0].second = massf(RBlist, vp, 0);
		offDipole * c = new offDipole(vp[0], &sim.offsetDipole, &sim.prevOffDipole, &sim.expenditure, ground, k, RBlist, angle, sim.rotationAxis); sim.condensedIntList[intIdx] = c; intIdx++;
	}
	else if (s == "LJstrength") sim.LJstrength[(int)ground + counter] = k;
	sim.condensedIntList[intIdx - 1]->min = min;
	sim.condensedIntList[intIdx - 1]->equil = equil;
	sim.condensedIntList[intIdx - 1]->umb = umb;
	sim.condensedIntList[intIdx - 1]->active = active;
	sim.condensedIntList[intIdx - 1]->isGene = isGene;
	sim.condensedIntList[intIdx - 1]->timeDependence = td;
	sim.condensedIntList[intIdx - 1]->tdPhaseOffset = tdpo;
	sim.condensedIntList[intIdx - 1]->tdPeriod = tdper;
}

void parseSuperIntCondensed(ifstream &ifs, string &s, vector<int> &RBlistCounter, simulation &sim, int &intIdx, int steps)
{
	rigidBody * RBlist = sim.RBlist;
	istringstream iss(s);
	...
	if (s == "constraint")
	{
		...
		constraint * c = new constraint(p[0], locus, ground, k, RBlist); sim.condensedIntList[intIdx] = c; intIdx++;
	}
	if (s == "constrainAxis")
	{
		getline(ifs, s); istringstream iss2(s);
		...
		p[0].first = vp[0].b + RBlistCounter[vp[0].a]; p[0].second = vp[0].c + 1;
		p[0].second = massf(RBlist, p, 0);
		constrainAxis * c = new constrainAxis(p[0], locus, ground, k, RBlist); sim.condensedIntList[intIdx] = c; intIdx++;
	}
	...
	else if (s == "bond" || s == "chain")
	{
		...
		bond * b = new bond(p[0], p[1], ground, k, RBlist); sim.condensedIntList[intIdx] = b; intIdx++;
		b->isChain = bisChain;
		bond temp = *b;
		sim.bondList.push_back(temp);
	}
	else if (s == "angle")
	{
		...
		angle * a = new angle(p[0], p[1], p[2], ground, k, RBlist); sim.condensedIntList[intIdx] = a; intIdx++;
	}
	else if (s == "dihedral")
	{
		iss >> divisor;
		...
		dihedral * d = new dihedral(p[0], p[1], p[2], p[3], ground, k, divisor, RBlist); sim.condensedIntList[intIdx] = d; intIdx++;
	}
	sim.condensedIntList[intIdx - 1]->min = min;
	sim.condensedIntList[intIdx - 1]->equil = equil;
	sim.condensedIntList[intIdx - 1]->umb = umb;
...
void parseFragCondensed(ifstream &ifs, vector3d &offset, int counter, quaternion &orient, simulation &sim, bool ind, triple &count, int steps)
{
	int i, j;
	string s;
	if (ind)
	{
		...
		while (true) //parse list of rigid bodies
		{
			if (s[0] != ';')
			{
				...
				if (s[0] == '!') break;
				//cout << s << '\n';
				if (s[0] == 'r' || s[0] == 'p' || s[0] == 'd' || (s[0] == 's' && s[1] == 'c')) sim.RBlistSize++;
			}
			getline(ifs, s);
		}
		getline(ifs, s);
		sim.conIntListSize = 0;
		if (s[0] != '!')
		{
			while (true) //parse list of interactions
			{
				...
				{
					if (s[0] == '!') break;
					probeInteraction(ifs, s, sim.conIntListSize);
				}
				getline(ifs, s);
			}
		}
		...
		sim.condensedIntList.resize(sim.conIntListSize);
	}
	...
	while (true) //parse list of rigid bodies
	{
		if (s[0] != ';')
		{
			//cout << s << '\n';
			if (s[0] == '!') break;
			if (s[0] == 'r' || (s[0] == 's' && s[1] == 'c'))
			{
				parseRigidCondensed(ifs, s, offset, counter, orient, sim, count.a);
				count.a++;
				//cout << count.a << '\n';
			}
			else if (s[0] == 'd')
	...
	if (s[0] != '!')
	{
		while (true) //parse list of interactions
		{
			...
			{
				if (s[0] == '!') break;
				parseIntCondensed(ifs, s, offset, counter, orient, sim, count.b, steps);
				//cout << count.b << '\n';
			}
			getline(ifs, s);
		}
	}
	cout << "condensed interactions done\n";
	getline(ifs, s);
	if (s[0] != '!')
	{
		while (true)
...
void parseSuperCondensed(ifstream &ifs, simulation &sim, int steps)
{
	vector<int> RBlistCounter; RBlistCounter.push_back(0);
	double a, b, c; int i = 0, j, k, l; sim.RBlistSize = 0; sim.conIntListSize = 0;
	string s;
	getline(ifs, s);
	while (true) //parse list of input files
	{
		...
		if (s[0] != ';')
		{
			...
			probeFrag(ifsf, sim.RBlistSize, sim.conIntListSize);
		}
		getline(ifs, s);
	}
	getline(ifs, s);
